...
function AssigneeChips({ assignees, max = 3 }: { assignees: TaskAssignee[]; max?: number }) {
  ...
  return (
    <div className="flex items-center">
      ...
      {rest > 0 && (
        <span className="text-[11.5px] font-bold h-5 w-5 rounded-full flex items-center justify-center border ml-[-4px]"
    ...
    >
      ...
      <div className="flex items-center gap-2 mt-2.5">
        ...
        {task.assignees.length > 0 && (
          <span className="text-[12.65px] truncate" style={{ color: colors.text.muted }}>
            {task.assignees[0].firstName}
            {task.assignees.length > 1 && ` +${task.assignees.length - 1} more`}
          </span>
        )}
        {due && (
          <span className="ml-auto flex items-center gap-1 text-[11.5px] font-medium flex-shrink-0"
      ...
      {(task.commentCount > 0 || task.attachmentCount > 0) && (
        <div className="flex items-center gap-3 mt-2 pt-2" style={{ borderTop: `1px solid ${border.divider}` }}>
          {task.commentCount > 0 && (
            <span className="flex items-center gap-1 text-[11.5px]" style={{ color: colors.text.dim }}>
      ...
      <Modal open title={modalTitle} onClose={onClose} size="lg">
        ...
        <div className="flex flex-wrap items-start gap-x-5 gap-y-3 mb-5 pb-5" style={{ borderBottom: `1px solid ${border.divider}` }}>
          ...
          <div className="flex flex-col gap-1 min-w-0">
            <span className="text-[11.5px] uppercase font-semibold tracking-wide" style={{ color: colors.text.dim }}>Status</span>
            {canEdit ? (
          ...
          {/* Priority */}
          <div className="flex flex-col gap-1 min-w-0">
            <span className="text-[11.5px] uppercase font-semibold tracking-wide" style={{ color: colors.text.dim }}>Priority</span>
            {canEditTask ? (
              <select
          ...
          <div className="flex flex-col gap-1 min-w-0">
            <div className="flex items-center gap-1">
              <span className="text-[11.5px] uppercase font-semibold tracking-wide" style={{ color: colors.text.dim }}>
                Assigned to
              </span>
              {canEditTask && (
            ...
            {task.assignees.length === 0 ? (
              ...
              <div className="flex flex-col gap-1">
                ...
                <button onClick={() => setShowAssigneeNames(false)}
                  className="text-[11.5px] text-left mt-0.5"
                  style={{ color: colors.accent }}>
                  collapse
                </button>
              </div>
          ...
          {due && (
            <div className="flex flex-col gap-1 min-w-0">
              <span className="text-[11.5px] uppercase font-semibold tracking-wide" style={{ color: colors.text.dim }}>Due</span>
                ...
                {logs.map((log: TaskLogResponse) => (
                  <div key={log.id} className="flex items-start gap-2.5">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[11.5px] mt-0.5" style={{ color: colors.text.dim }}>
          ...
          {COLUMNS.map(col => {
            ...
            return (
              <button key={col.status} onClick={() => setMobileTab(col.status)}
                ...
                {col.label}
                <span className="text-[11.5px] font-bold px-1 py-0.5 rounded-full"
